...
## Create a Data Frame with all 4 Samples

Create the following columns in each data frame: 

sample_name = the name of the sample

virus_type = contig or vMAG

sample_depth = the depth at which the sample was taken at

Then rename each data frame's columns to match each other. Merge data frames together into a larger data frame called 'combo'.

```{r}
jv119_contigs$sample_name <- "JV119" # create sample_name column
jv119_contigs$virus_type <- "contig" # create virus_type column
jv119_contigs$sample_depth <- 119    # create sample_depth column

jv119_vMAG$sample <- "JV119"
jv119_vMAG$element <- "vMAG"
jv119_vMAG$sample_depth <- 119

jv121_contigs$sample <- "JV121"
jv121_contigs$element <- "contig"
jv121_contigs$sample_depth <- 121

jv121_vMAG$sample <- "JV121"
jv121_vMAG$element <- "vMAG"
jv121_vMAG$sample_depth <- 121

jv132_contigs$sample <- "JV132"
jv132_contigs$element <- "contig"
jv132_contigs$sample_depth <- 132

jv132_vMAG$sample <- "JV132"
jv132_vMAG$element <- "vMAG"
jv132_vMAG$sample_depth <- 132

jv154_contigs$sample <- "JV154"
jv154_contigs$element <- "contig"
jv154_contigs$sample_depth <- 154

jv154_vMAG$sample <- "JV154"
jv154_vMAG$element <- "vMAG"
jv154_vMAG$sample_depth <- 154

df_list <- list(jv119_contigs, jv119_vMAG, jv121_contigs, jv121_vMAG, jv132_contigs, jv132_vMAG, jv154_contigs, jv154_vMAG) # create a list of all of the data frames
new_names <- c("virus_name", "virus_length", "virus_read_count", "virus_read_depth", "virus_read_depth_in_host", "host_name","host_length", "host_read_count", "host_read_depth", "intra_read_count", "intra_linkage_density", "inter_read_count", "raw_inter_linkage_density", "raw_inter_vs_intra_ratio", "viral_copies_per_cell", "adjusted_inter_linkage_density", "adjusted_inter_vs_intra_ratio", "sample", "element", "sample_depth") # create a list of the new column names

for (i in seq_along(df_list)) {
  colnames(df_list[[i]]) <- new_names
} # for loop to rename the columns in all of the data frames

jv119_contigs <- do.call(data.frame, df_list[[1]]) # extract data frame from the data frame list
jv119_vMAG <- do.call(data.frame, df_list[[2]])
jv121_contigs <- do.call(data.frame, df_list[[3]])
jv121_vMAG <- do.call(data.frame, df_list[[4]])
jv132_contigs <- do.call(data.frame, df_list[[5]])
jv132_vMAG <- do.call(data.frame, df_list[[6]])
jv154_contigs <- do.call(data.frame, df_list[[7]])
jv154_vMAG <- do.call(data.frame, df_list[[8]])
